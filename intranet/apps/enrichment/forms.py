...
class EnrichmentActivityBulkForm(EnrichmentActivityForm):
    ...
    week_of = forms.DateField(
        label="Target Week",
        widget=forms.DateInput(attrs={"class": "datetimepicker-date"}),
    )
    ...
    def get_selected_dates(self) -> list[datetime.datetime]:
    ...
    def create_for_date(self, dt: datetime.datetime, user) -> EnrichmentActivity:
        """Create one EnrichmentActivity for the given datetime, using this form's cleaned data."""
        activity = self.save(commit=False)
        activity.pk = None  # Reset the primary key because we're using one form to make multiple enrichments.
        activity.user = user
        activity.time = dt
